...
import org.webrtc.audio.AudioDeviceModule
        import com.buligin.vishnucast.audio.NativeAdm                 // B2-native: ADM из so для PLAYER
import com.buligin.vishnucast.audio.WebRtcSource              // План B: MIC / PLAYER
...
// План B2-native: выбираем ADM в зависимости от источника (MIC/PLAYER)
...
/**
 * Фабрика ADM:
 * - MIC: стандартный JavaAudioDeviceModule (VOICE_RECOGNITION, HW AEC/NS off, SW on).
 * - PLAYER: нативный ADM из so (если нет — фоллбэк на Java ADM, чтобы не ломать запуск).
 */
private fun buildAudioDeviceModule(appContext: Context): AudioDeviceModule {
return if (WEBRTC_SOURCE == WebRtcSource.PLAYER) {
NativeAdm.createPlayerOnlyAdm(appContext)
?: run {
w("Native ADM unavailable, falling back to Java ADM (MIC)")
...
// --- (Ниже вспомогалки; могут быть неиспользованы, оставляем для совместимости с прошлым кодом) ---
...
companion object {
        ...
        /** Флаг каркаса Mix 2.0 (сейчас не влияет). */
        ...
        /** План B: источник WebRTC (жёсткий переключатель). */
        @Volatile var WEBRTC_SOURCE: WebRtcSource = WebRtcSource.MIC

        fun setWebRtcSource(src: WebRtcSource) {
            WEBRTC_SOURCE = src
